...
public interface UserRepo extends JpaRepository<User, Long> {
	...
	@Query(value = "SELECT adm_user.*,EMS_expert.*,\r\n"
			+ "        round(months_between(TRUNC(sysdate),\r\n"
			+ "        adm_user.birth_date                       )/12,\r\n"
			+ "        0)   as EXPERT_AGE   FROM adm_user,EMS_expert WHERE adm_user.USER_ID=EMS_expert.USER_ID(+) and USER_NAME = ?1", nativeQuery = true)
